...
const sessionRandom = (req, res, next) => {
    req.session.random = `${Math.round(Math.random() * 200000)}`;
    next();
};
videoRouter.route("/upload").all(middlewares_1.protectOnlyMiddleware).get(videoController_1.getUpload).post(sessionRandom, middlewares_1.uploadVideo.single("video"), videoController_1.postUpload);
