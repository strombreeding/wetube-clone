...
const handleSubmit=(e:Event)=>{
    ...
    const user = document.getElementsByClassName("header__avatar")[0] as HTMLImageElement
    if(text.length >= 2){
        $.ajax({
            type:"POST",
            ...
            success : (res)=>{
                ...
                commentList?.prepend(cmtContainer)
            }   
        })
    }else{
        alert("2글자 이상 적어주세요.")
...
const showCommentBtn =()=>{
    const addCommentBtn = document.getElementById("addCommentBtn") as HTMLInputElement
